 ...
 *
 * The Phase bar is deliberately NOT a coloured bar here (colour is reserved
 * for RJF/Client blocks) - instead each day cell that falls inside a phase
 * just gets the phase's name appended next to its date number, in small
 * light-grey text (e.g. "06 Design phase"). Admins still drag/resize phase
...
import type { PhaseBarEntry, ScheduleBlock } from "@/lib/storage/types";
...
/** The phase (if any) covering a given day, used to label the date number - not rendered as a coloured bar here. */
  ...
  return (
    <div className="relative flex border-b border-r">
      {days.map((day) => {
        ...
        const phase = phaseForDay(phaseEntries, day);
        return (
          <div
            key={day}
            className={cn(
          ...
          >
            <div className="flex items-baseline gap-1">
              ...
              {phase && <span className="min-w-0 truncate text-[10px] text-muted-foreground/70">{phase.label}</span>}
            </div>
          </div>
        );
      })}
